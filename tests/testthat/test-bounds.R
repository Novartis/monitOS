...
test_that("Table #2: POLARIX ", {
  ...
  expect_equal(as.numeric(res$summary$Positivity_Thres_HR),  # OS HR threshold for positivity
               c(1.114,  1.050, 1.021, 1.001,  0.969),
               tolerance=TOLERANCE)
  expect_equal(as.numeric(res$summary$OneSided_falsepos),  # One-sided false positive error rate
               c(0.275, 0.157, 0.103, 0.067, 0.025),
               tolerance=TOLERANCE)
  expect_equal(as.numeric(res$summary$TwoSided_CI_level),  # Level of 2-sided CI needed to rule out
               c(45, 69, 79, 87, 95),
               tolerance=1e-1)
...
test_that("Table#3: Intermediate events", {
  ...
  expect_equal(as.numeric(res$summary$Power_Alt),  # Probability of meeting positivity threshold under δalt
               c(0.9, 0.9, 0.905),
               tolerance=TOLERANCE)
  expect_equal(as.numeric(res$summary$Power_Marg_Benefit),  # Probability of meeting positivity threshold under HR = 0.95
               c(0.682, 0.615 , 0.512),
               tolerance=TOLERANCE)
})
...
test_that("Table#4: Example 2", {
  ...
  expect_equal(as.numeric(res$summary$Power_Alt),  # Probability of meeting positivity threshold under δalt
               c(0.750, 0.750, 0.681),
               tolerance=TOLERANCE)
  expect_equal(as.numeric(res$summary$Power_Marg_Benefit),  # Probability of meeting positivity threshold under HR = 1
               c(0.705 , 0.694, 0.601),
               tolerance=TOLERANCE)
})
