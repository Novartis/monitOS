...
app_server <- function(input, output, session) {
  ...
  react <- reactive({
    ...
    # boundaries
    boundaries = bounds(events = events,
  ...
  # Rendering
  output$bounds <- renderTable(react()$summary %>%
                                 select(-c(Delta_null, Delta_alt, Positivity_Thres_Posterior, Positivity_Thres_PredProb)) %>%
                                 ...
                                 rename('Probability of meeting positivity threshold under marginal HR' = Power_Marg_Benefit))
}
